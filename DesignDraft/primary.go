...
func PrimaryRoutine(StateUpdateCh) {
	...
	go run HandlePrimaryTasks(StateUpdateCh)		  //take in conns to use in GetAllActiveElevatorStates -> må bli en channel for å sende mellom TCPListenForNewElevator updates
}
...
*/
func HandlePrimaryTasks(StateUpdateCh) {
	var backup Elevator
	var ActiveElevators []Elevator

	for {
		stateUpdate <- StateUpdateCh					//updates if new state is sendt on one of TCP conns, blocks if not
		//TODO: compare the state update from single elevator to old elevatorStates array and update it
